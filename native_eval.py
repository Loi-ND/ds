...
from query.core import get_llm, get_embedding_model
from query.medical.medical_rag import MedicalNativeRAG
...
CHECKPOINT_FILE = "naive_eval_results_checkpoint.json"
FINAL_RESULT_FILE = "naive_eval_results.json"
...
def evaluate_rag(
    ...
    logger.info("🚀 START evaluate_rag")
    logger.info(f"Eval file: {eval_path} | k={k}")
    ...
    # ===== INIT MODELS =====
    logger.info("[INIT] Load LLMs & models")

    # 🔹 Answer LLM: TEXT ONLY (NO structured output)
    answer_llm = get_llm("llama3")

    embedding_model = get_embedding_model()
    medical_rag = MedicalNativeRAG(embedder=embedding_model)
    ...
    for idx, sample in enumerate(eval_data):
        ...
        try:
            ...
            hits = medical_rag.query(query, limit=k)
            time.sleep(1)  # giảm áp lực Qdrant
    ...
    logger.info("✅ FINISH evaluate_rag")
    return results
